...
class TiaaPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('tiaa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `tiaa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM tiaa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO tiaa (date, title, link, content)"
